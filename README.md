...
启动后会询问：`是否汉化 x-ui 命令菜单? (y/N)`（只动 `x-ui` 命令行脚本，不影响 Web 面板）

汉化范围是整个命令行脚本：主菜单、子命令用法、二级菜单（日志/防火墙/证书/IP 限制/PostgreSQL 等），
以及运行过程中的提示、确认、报错和输入提示。

- 按文案匹配，不绑定菜单编号，上游改版重排菜单后依然生效
- 边框菜单按中文显示宽度重新对齐，不会出现边框错位
- 命令、路径、DSN、内核参数等技术标识保持英文
...
curl -fsSL -o xui_cf_deployer.py [https://raw.githubusercontent.com/byJoey/xui-cf-deployer/main/xui_cf_deployer.py](https://raw.githubusercontent.com/Nickname99987/xui-cf-deployer/refs/heads/main/xui_cf_deployer.py)
